...
# Modifications done by Edoardo Morassutto <edoardo.morassutto@gmail.com>
# Source: https://github.com/trainman419/bazel/blob/6123cb2673dccee783c80086ba7d217c4f721922/tools/build_defs/pkg/pkg.bzl
# Removed some of the actions for middleman because we don't use such feature
# Added package stripping to reduce library and executable sizes
...
def _strip_tar_impl(ctx):
  temp = ctx.actions.declare_directory("temp_root")
  input = ctx.file.data
  output = ctx.outputs.out
  ctx.actions.run_shell(
      outputs=[temp],
      command="mkdir -p %s" % temp.path,
      mnemonic="CreateDir",
  )
  ctx.actions.run_shell(
    inputs=[input, temp],
    outputs=[output],
    progress_message="Stripping %s" % input.short_path,
    command="""
      chmod 755 %s &&
      tar -xf %s -C %s &&
...
def _pkg_tar_impl(ctx):
  ...
  files = list(depset(ctx.files.files) + runfiles)
