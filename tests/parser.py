...
class TestParser(unittest.TestCase):
    ...
    def test_mismatch_cases(self):
        for i in range(2):
            with self.subTest(case_number=i):
                p = Parser(lambda x, y: int(x == y.int()), 1,
                           io.StringIO("Case #42: 42\nCase #1: " + str(i)))
    ...
    def test_too_long_string(self):
        p = Parser(lambda x, y: float(y.str() == "wibblemonster"), 1,
                   io.StringIO("Case #1: wibblemonster"), str_max_len=5,
                   int_max_len=5, float_max_len=5)
    ...
    def test_same_cases(self):
        p = Parser(lambda x, y: float(y.int() == 0), 1,
                   io.StringIO("Case #1: 1\nCase #1: 0"))
        self.assertEqual(p.run()["score"], 0.0)

    def test_same_space_separated(self):
        p = Parser(lambda x, y: float(y.int() == 0), 1,
                   io.StringIO("Case #1: 0 Case #1: 1"))
        self.assertEqual(p.run()["score"], 1.0)

    def test_same_invalid_separated(self):
        p = Parser(lambda x, y: float(y.int() == 0), 1,
                   io.StringIO("Case #1: 0Case #1: 0"))
        self.assertEqual(p.run()["score"], 0.0)

    def test_wrong_case_naming(self):
        p = Parser(lambda x, y: float(y.int() == 0), 1,
                   io.StringIO("Caze #1: 0"))
        self.assertEqual(p.run()["score"], 0.0)
    ...
    def test_case_too_long(self):
        p = Parser(lambda x, y: 1.0, 1,
                   io.StringIO("Case #" + str(2 ** 2 ** 2 ** 2 ** 2) + ": "))
        self.assertEqual(p.run()["score"], 0.0)
    ...
    def test_strict_has(self):
        ...
        p = Parser(tester, 1, io.StringIO("Case #1: foobar"),
                   strict_spaces=True)
        self.assertEqual(p.run()["score"], 1.0)
    ...
    def test_int_lower(self):
        with self.assertRaises(ValueError):
            p = Parser(lambda x, y: float(y.int() == 42), 1,
                       io.StringIO("Case #1: 42"), str_max_len=1)
            self.assertEqual(p.run()["score"], 1.0)
...
class FuzzerParser(unittest.TestCase):
    def test_int(self):
        for i in range(FUZZER_RUNS):
            ...
            with self.subTest(run=i, fuzz=fuzz):
                p = Parser(lambda x, y: float(y.int() == -1), 1,
                           io.StringIO("Case #1: " + fuzz))
                self.assertEqual(p.run()["score"], 0.0)
    ...
    def test_int_parameters(self):
        for i in range(FUZZER_RUNS):
            ...
            with self.subTest(run=i, fuzz=fuzz, int_param=k):
                p = Parser(lambda x, y: float(y.int() == -1), 1,
                           io.StringIO("Case #1: " + fuzz), int_max_len=k)
                self.assertEqual(p.run()["score"], 0.0)

    def test_float(self):
        for i in range(FUZZER_RUNS):
            fuzz = "".join(
                choices(string.digits, k=randrange(FUZZER_LENGTH // 2)))
            fuzz += "."
            fuzz += "".join(
                choices(string.digits, k=randrange(FUZZER_LENGTH // 2)))
            with self.subTest(run=i, fuzz=fuzz):
                p = Parser(lambda x, y: float(y.float() == -1.0), 1,
                           io.StringIO("Case #1: " + fuzz))
                self.assertEqual(p.run()["score"], 0.0)
    ...
    def test_float_parameters(self):
        for i in range(FUZZER_RUNS):
            ...
            with self.subTest(run=i, fuzz=fuzz, float_param=k):
                p = Parser(lambda x, y: float(y.float() == -1.0), 1,
                           io.StringIO("Case #1: " + fuzz), float_max_len=k)
                self.assertEqual(p.run()["score"], 0.0)
    ...
    def test_string(self):
        for i in range(FUZZER_RUNS):
            fuzz = "".join(choices(
                ...
                k=randrange(FUZZER_LENGTH + 1)))
            with self.subTest(run=i, fuzz=fuzz):
                p = Parser(lambda x, y: float(y.str() == ""), 1,
                           io.StringIO("Case #1:" + fuzz), strict_spaces=True)
                self.assertEqual(p.run()["score"], 0.0)
    ...
    def test_string_parameters(self):
        for i in range(FUZZER_RUNS):
            fuzz = "".join(choices(
                string.digits + string.ascii_uppercase + string.ascii_lowercase,
                k=randrange(FUZZER_LENGTH + 1)))
            k = max(randrange(FUZZER_LENGTH), DEFAULT_INT_MAX_LEN,
                    DEFAULT_FLOAT_MAX_LEN)
            with self.subTest(run=i, fuzz=fuzz, string_param=k):
                p = Parser(lambda x, y: float(y.str() == ""), 1,
                           io.StringIO("Case #1:" + fuzz), str_max_len=k,
                           strict_spaces=True)
                self.assertEqual(p.run()["score"], 0.0)
    ...
    def test_raw(self):
        for i in range(FUZZER_RUNS):
            fuzz = "".join(
                choices(string.printable, k=randrange(FUZZER_LENGTH)))
